...
class App {
    ...
    private validaDadosPerfil(nome?: string, email?: string): void {
        ...
        if (!email.includes("@")) {
            throw new InputInvalidoError(
                "Email inválido: o email deve conter um '@'."
            );
        }
        ...
        if (email.indexOf("@") > email.lastIndexOf(".")) {
            throw new InputInvalidoError(
                "Email inválido: o '@' deve estar antes do '.'."
            );
        }

        if (email.indexOf("@") === 0) {
            throw new InputInvalidoError(
                "Email inválido: o '@' não pode ser o primeiro caractere."
            );
        }

        if (email.lastIndexOf(".") === email.length - 1) {
            throw new InputInvalidoError(
                "Email inválido: o '.' não pode ser o último caractere."
            );
        }

        if (email.lastIndexOf("@") !== email.indexOf("@")) {
            throw new InputInvalidoError(
                "Email inválido: o email não pode conter mais de um '@'."
            );
        }

        if (email.lastIndexOf(".") < email.indexOf("@")) {
            throw new InputInvalidoError(
                "Email inválido: o '.' deve estar depois do '@'."
            );
        }

        if (email.lastIndexOf(".") - email.indexOf("@") === 1) {
            throw new InputInvalidoError(
                "Email inválido: o '.' não pode estar logo depois do '@'."
            );
        }
    }

    private validaDadosPostagem(texto: string): void {
    ...
    carregarPerfis(): void {
        try {
            ...
            for (let perfil of perfisFile) {
                ...
                const [id, nome, email]: string[] = dadosPerfil;
            ...
            for (const postagem of postagensFile) {
                ...
                if (trimmedPostagem != "") {
                    ...
                    let [
                        ...
                        perfilId,
                        hashtags,
                        vizualizacoesRestantes,
                    ] = dadosPostagem;
                    const hashtagArray = (hashtags || "").split(",");
                    let novaPostagem = new Postagem(
                        id,
                        texto,
                        Number(curtidas),
                        Number(descurtidas),
                    ...
                    if (tipo == "pa") {
                        novaPostagem = new PostagemAvancada(
                            id,
                            texto,
                            Number(curtidas),
                            Number(descurtidas),
    ...
    editarPerfil(): void {
        ...
        if (oqueEditar == 'nome') {
            ...
            throw new InputInvalidoError(
                "Opção inválida: a opção deve ser nome ou email."
            );
        }
